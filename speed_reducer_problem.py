...
class SpeedReducerProblem:
    ...
    def __init__(self):
        ...
        self.dim = self.n_dim
        self.bounds_lower = self.bounds_array[:, 0]
        self.bounds_upper = self.bounds_array[:, 1]
        
        # For BoTorch compatibility (normalized bounds)
        import torch
        self.bounds = torch.tensor([[0.0] * self.n_dim, [1.0] * self.n_dim], 
                                   dtype=torch.float64)
        
    def _denormalize(self, x: np.ndarray) -> np.ndarray:
        """Convert from [0, 1] to original bounds"""
        return self.bounds_lower + x * (self.bounds_upper - self.bounds_lower)
    
    def _normalize(self, x: np.ndarray) -> np.ndarray:
        """Convert from original bounds to [0, 1]"""
        return (x - self.bounds_lower) / (self.bounds_upper - self.bounds_lower)
    
    def evaluate(self, x, normalized=False):
        """
        ...
        normalized : bool, optional
            If True, x is in [0, 1] and needs to be denormalized
        ...
        # g10: Shaft 1 geometry constraint: (1.5*x6 + 1.9)/x4 <= 1
        c[9] = (1.5 * x6 + 1.9) / x4 - 1
        
        # g11: Shaft 2 geometry constraint: (1.1*x7 + 1.9)/x5 <= 1
...
def test_speed_reducer():
    ...
    for i, (lb, ub) in enumerate(problem.bounds_array):
